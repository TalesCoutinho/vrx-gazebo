...
export GZ_SIM_RESOURCE_PATH=$GZ_SIM_RESOURCE_PATH:"$WORKSPACE_DIR"/vrx/vrx_urdf:"$WORKSPACE_DIR"/vrx/vrx_gz/models
...
xacro "$WORKSPACE_DIR"/vrx/vrx_urdf/wamv_gazebo/urdf/wamv_gazebo.urdf.xacro namespace:=wamv locked:=true vrx_sensors_enabled:=false gps_enabled:=true imu_enabled:=true lidar_enabled:=true bathymetry_enabled:=true thruster_config:=H > $URDF_PATH
...
ros2 launch vrx_gz competition.launch.py headless:=false world:=bathymetry urdf:=$URDF_PATH vrx_sensors_enabled:=false gps_enabled:=true imu_enabled:=true lidar_enabled:=true bathymetry_enabled:=true > "$WORKSPACE_DIR"/log/gazebo_log.txt 2>&1 &
SIM_PID=$!
echo "Aguardando 35 segundos para o Gazebo carregar minimamente..."
sleep 35

echo "Coletando árvore de TFs..."
ros2 run tf2_tools view_frames -o "$WORKSPACE_DIR"/log/tf_tree
...
# python3 "$WORKSPACE_DIR"/fake_lidar.py &

ros2 launch robmov_slam slam_localization.launch.py > "$WORKSPACE_DIR"/log/slam_log.txt 2>&1 &
SLAM_PID=$!
sleep 25

echo "Testando sensores (salvando em sensors_log.txt)..."
ros2 topic hz /wamv/sensors/imu/imu/data --window 5 > "$WORKSPACE_DIR"/log/sensors_log.txt 2>&1 &
ros2 topic hz /wamv/sensors/gps/gps/fix --window 5 >> "$WORKSPACE_DIR"/log/sensors_log.txt 2>&1 &
ros2 topic hz /wamv/sensors/lidars/lidar_wamv_sensor/scan --window 5 >> "$WORKSPACE_DIR"/log/sensors_log.txt 2>&1 &

echo "Iniciando RViz para acompanhamento da Navegação..."
rviz2 -d /opt/ros/jazzy/share/nav2_bringup/rviz/nav2_default_view.rviz > "$WORKSPACE_DIR"/log/rviz_log.txt 2>&1 &
RVIZ_PID=$!

echo "Iniciando Coletor de Batimetria com Log e Anomalia..."
ros2 run robmov_slam software_bathymetry.py > "$WORKSPACE_DIR"/log/software_bathy.log 2>&1 &
MOCK_PID=$!
ros2 run robmov_slam bathymetry_mapper.py > "$WORKSPACE_DIR"/log/bathymetry_syslog.txt 2>&1 &
BATHY_PID=$!
sleep 25
...
python3 -u "$WORKSPACE_DIR"/tracker.py > "$WORKSPACE_DIR"/log/tracker_log.txt 2>&1 &
