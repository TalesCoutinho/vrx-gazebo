...
ros2 launch vrx_gz competition.launch.py headless:=true world:=bathymetry urdf:=$URDF_PATH vrx_sensors_enabled:=false gps_enabled:=true imu_enabled:=true lidar_enabled:=true bathymetry_enabled:=true > "$WORKSPACE_DIR"/gazebo_log.txt 2>&1 &
SIM_PID=$!
echo "Aguardando 35 segundos para o Gazebo carregar minimamente..."
sleep 35

echo "Coletando árvore de TFs..."
ros2 run tf2_tools view_frames -o "$WORKSPACE_DIR"/tf_tree
